...
// Repo root (apps/web/scripts -> ../../.. = repo root)
 ...
 */
function buildIntentDisplay(q) {
  if (q.natural_language_query && q.natural_language_query.trim()) return q.natural_language_query.trim();
  if (q.intent && q.intent.trim()) return q.intent.trim();
  const parts = [];
  if (q.use_case && q.use_case.trim()) parts.push(q.use_case.trim());
  if (q.business_value && q.business_value.trim()) parts.push(q.business_value.trim());
  if (q.purpose && q.purpose.trim()) parts.push(q.purpose.trim());
...
function extractQueryInfo(queries) {
  ...
  return {
    ...
    queries: queries.map(q => ({
      ...
      purpose: q.purpose || '',
      intent_display: buildIntentDisplay(q),
...
function extractDatabase(dbNum) {
  ...
  return {
    ...
    queries: {
      ...
      preview: queries.queries.slice(0, 5),
      queries: queries.queries // Full list for Qdrant sync
    },
    files: {
