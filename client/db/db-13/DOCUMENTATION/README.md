...
- `total_parameters_billions` NUMERIC(10  — Total parameters (for open weights)
- `active_parameters_billions` NUMERIC(10  — Active parameters at inference (MoE models)
- `model_type` VARCHAR(50)  — 'dense', 'moe', 'reasoning', 'non_reasoning'
- `architecture_type` VARCHAR(100)  — Transformer architecture details
- `training_data_size_tokens` NUMERIC(20  — Training data size
- `training_compute_pflops` NUMERIC(20  — Training compute in petaFLOPs
...
- `intelligence_index_score` NUMERIC(10  — Artificial Analysis Intelligence Index v4.0
- `intelligence_index_percentile` NUMERIC(5  — Percentile ranking
- `coding_index_score` NUMERIC(10  — Coding-specific performance
- `agentic_index_score` NUMERIC(10  — Agentic capabilities score
- `output_speed_tokens_per_sec` NUMERIC(10  — Output tokens per second
- `latency_seconds` NUMERIC(10  — Time to first token
- `end_to_end_response_time_500tokens` NUMERIC(10  — Seconds to output 500 tokens
- `input_price_per_million_tokens` NUMERIC(10  — USD per 1M input tokens
- `output_price_per_million_tokens` NUMERIC(10  — USD per 1M output tokens
- `blended_price_per_million_tokens` NUMERIC(10  — Blended price (3:1 input:output ratio)
- `reasoning_price_per_million_tokens` NUMERIC(10  — Reasoning tokens price (if applicable)
- `openness_index` NUMERIC(5  — Openness Index (0-100)
- `omniscience_index` NUMERIC(10  — AA-Omniscience Index (-100 to 100)
- `omniscience_accuracy` NUMERIC(5  — Accuracy percentage
- `omniscience_hallucination_rate` NUMERIC(5  — Hallucination rate percentage
- `evaluation_version` VARCHAR(50)  — Evaluation framework version
- `data_source` VARCHAR(100) 
- `created_at` TIMESTAMP 
- `updated_at` TIMESTAMP 
- `FOREIGN` KEY (model_id) FOREIGN KEY
...
- `evaluation_date` DATE NOT NULL
- `score` NUMERIC(10  — Raw score
- `normalized_score` NUMERIC(10  — Normalized score (0-100 or percentile)
- `percentile_rank` NUMERIC(5  — Percentile ranking
...
- `accuracy_percentage` NUMERIC(5  — Accuracy percentage
...
- `model_1_score` NUMERIC(10 
- `model_2_score` NUMERIC(10 
- `winner_model_id` VARCHAR(255)  — Model with better score
- `score_difference` NUMERIC(10  — Absolute difference
- `score_difference_percent` NUMERIC(5  — Percentage difference
- `comparison_context` VARCHAR(500)  — Context of comparison
- `data_source` VARCHAR(100) 
- `created_at` TIMESTAMP 
- `FOREIGN` KEY (model_id_1) FOREIGN KEY
- `FOREIGN` KEY (model_id_2) FOREIGN KEY
- `FOREIGN` KEY (winner_model_id) FOREIGN KEY
...
- `market_share_percentage` NUMERIC(5  — Market share percentage
- `market_position` VARCHAR(50)  — 'leader', 'challenger', 'follower', 'niche'
- `average_intelligence_score` NUMERIC(10  — Average intelligence score
- `average_price_per_million_tokens` NUMERIC(10  — Average price
- `average_speed_tokens_per_sec` NUMERIC(10  — Average speed
- `model_count` INTEGER  — Number of models in segment
- `growth_rate_percent` NUMERIC(5  — Growth rate percentage
...
- `score` NUMERIC(10  — Benchmark score
...
- `safety_score` NUMERIC(5  — Safety score (if applicable)
- `robustness_score` NUMERIC(5  — Robustness score (if applicable)
- `bias_score` NUMERIC(5  — Bias score (if applicable)
- `efficiency_score` NUMERIC(5  — Efficiency score (if applicable)
...
- `api_calls_millions` NUMERIC(10  — Estimated API calls in millions
- `active_users_thousands` INTEGER  — Estimated active users in thousands
- `market_penetration_percent` NUMERIC(5  — Market penetration percentage
- `developer_sentiment_score` NUMERIC(5  — Sentiment score (-100 to 100)
...
- `industry_adoption_score` NUMERIC(5  — Industry adoption score (0-100)
- `enterprise_adoption_percent` NUMERIC(5  — Enterprise adoption percentage
...
- `input_price_per_million_tokens` NUMERIC(10 
- `output_price_per_million_tokens` NUMERIC(10 
- `blended_price_per_million_tokens` NUMERIC(10 
- `reasoning_price_per_million_tokens` NUMERIC(10 
- `cached_price_per_million_tokens` NUMERIC(10  — Cached pricing (if applicable)
- `image_input_price_per_million_tokens` NUMERIC(10  — Image input pricing
- `price_change_percent` NUMERIC(5  — Price change percentage from previous period
...
- `intelligence_index_score` NUMERIC(10 
- `output_speed_tokens_per_sec` NUMERIC(10 
- `latency_seconds` NUMERIC(10 
- `performance_change_percent` NUMERIC(5  — Performance change from previous period
...
- `data_quality_score` NUMERIC(5  — Data quality score (0-100)
- `is_active` BOOLEAN 
