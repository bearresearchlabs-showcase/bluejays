...
```yaml
db_id: db-4
domain: AI/ML, model registry, model analytics
source: synthetic
license_type: Open
license_cost: $0
tables: 1
...
```text
This database supports analytics for AI model usage: model altitude (id), time-series patterns, rolling averages, and outlier detection.
```

## Use Case

```text
Target use cases: model usage dashboards, time-series analytics, z-score outlier detection, trend analysis.
```

## Business Value

```text
Enables ML teams to monitor model deployment patterns and identify anomalies in model usage ($1M+ ARR).
```
...
```text
models table (id, name, user_id, created_at). Window functions for rolling avg, LAG/LEAD, z-scores.
```
