...
1. [Query 1: Can you show me a multi-dimensional analysis that compares my credit card portfolio and calculates how much I'm missing out on in rewards?](#query-1)
    ...
    - *What it does:* Situation: A credit card user maintains multiple cards with varying reward structures across different merchants and spending categories. They want to...
...
2. [Query 2: Can you recommend which credit cards I should use at nearby merchants based on my location and show me the best rewards opportunities around me?](#query-2)
    ...
    - *What it does:* Situation: A credit card user is planning shopping trips or evaluating spending opportunities in their geographic area. They want to know which nearby...
...
3. [Query 3: Can you show me all available bank offers across my credit cards, tell me which ones I'm eligible for, and help me prioritize which offers to activate for maximum savings?](#query-3)
    ...
    - *What it does:* Situation: Credit card issuers frequently provide targeted offers such as cash back bonuses, statement credits, or elevated rewards at specific mercha...
...
4. [Query 4: Can you analyze consumer complaint data from the CFPB to assess which credit card issuers have the highest risk profiles and help me avoid problematic cards?](#query-4)
    ...
    - *What it does:* Situation: Credit card users want to make informed decisions not just based on rewards and fees, but also on issuer reliability and customer service q...
...
5. [Query 5: Can you analyze Federal Reserve credit data to show market trends, identify different consumer segments, and provide predictive indicators for the credit card industry?](#query-5)
    ...
    - *What it does:* Situation: The Federal Reserve publishes aggregate consumer credit data including revolving credit balances, delinquency rates, charge-off rates, and...
...
6. [Query 6: Can you show me Chase 5/24 rule tracking with application strategy optimization and timing recommendations?](#query-6)
    ...
    - *What it does:* Situation: Credit card enthusiasts managing multiple card applications need to track their eligibility under Chase's 5/24 rule, which denies applicati...
    - *Business Value:* [Description of expected output]
...
7. [Query 7: Can you show me annual fee optimization with card renewal value analysis and portfolio cost-benefit calculation?](#query-7)
    ...
    - *What it does:* Situation: Credit card holders with multiple premium cards face significant annual fee burdens that may not be justified by the rewards and benefits e...
    - *Business Value:* [Description of expected output]
...
8. [Query 8: Can you show me signup bonus tracking and optimization with minimum spend requirement analysis and timing recommendations?](#query-8)
    ...
    - *What it does:* Situation: New credit card signups typically offer lucrative signup bonuses contingent on meeting minimum spend requirements within specified timefram...
    - *Business Value:* [Description of expected output]
...
9. [Query 9: Can you show me merchant-specific card recommendations with historical spending pattern analysis and predictive scoring?](#query-9)
    ...
    - *What it does:* Situation: Credit card users want to maximize rewards by using the optimal card for each merchant based on category bonuses, promotional rates, and sp...
    - *Business Value:* [Description of expected output]
...
10. [Query 10: Can you show me category bonus period optimization with quarterly rotation analysis and spending allocation strategy?](#query-10)
    ...
    - *What it does:* Situation: Several credit cards offer rotating quarterly bonus categories (e.g., 5% cash back on up to $1,500 in combined purchases in categories that...
    - *Business Value:* [Description of expected output]
...
11. [Query 11: Can you show me how our spending is distributed across different categories and where we might be able to optimize our expenses?](#query-11)
    ...
    - *What it does:* Situation: The finance team is reviewing quarterly credit card expenses to identify cost reduction opportunities. With thousands of transactions acros...
...
12. [Query 12: Can you analyze our credit card portfolio to show how diversified our card usage is across different card types?](#query-12)
    ...
    - *What it does:* Situation: The consumer credit team manages a diverse portfolio of credit card products and needs to assess whether customers are appropriately divers...
    - *Business Value:* [Expected output description]
...
13. [Query 13: Can you help me understand how much we're paying in foreign transaction fees and identify opportunities to reduce these costs?](#query-13)
    ...
    - *What it does:* Situation: As international spending increases among cardholders, foreign transaction fees (typically 1-3% per transaction) have become a significant...
    - *Business Value:* [Expected output description]
...
14. [Query 14: Can you analyze the fees we're paying for authorized users and determine if these costs are justified by the usage patterns?](#query-14)
    ...
    - *What it does:* Situation: Many premium credit cards charge annual fees for authorized users (additional cardholders on an account), ranging from $0 to $175+ per user...
    - *Business Value:* [Expected output description]
...
15. [Query 15: Can you show me how we're using our available credit limits and identify opportunities to optimize our credit utilization?](#query-15)
    ...
    - *What it does:* Situation: Credit utilization (the percentage of available credit being used) is a critical metric that affects credit scores and borrowing costs. The...
    - *Business Value:* [Expected output description]
...
16. [Query 16: Can you show me an analysis of how customers are redeeming their rewards points?](#query-16)
    ...
    - *What it does:* Situation: The rewards program manager needs to understand how cardholders are utilizing their accumulated points across various redemption channels (...
    - *Business Value:* [Expected output description]
...
17. [Query 17: Can you provide me with an analysis of our transfer partner program performance?](#query-17)
    ...
    - *What it does:* Situation: The partnerships team manages relationships with airline, hotel, and retail transfer partners where cardholders can convert reward points a...
    - *Business Value:* [Expected output description]
...
18. [Query 18: Can you generate card upgrade and downgrade recommendations for our customers?](#query-18)
    ...
    - *What it does:* Situation: The customer retention and growth team wants to proactively identify cardholders whose spending behavior suggests they would derive more va...
    - *Business Value:* [Expected output description]
...
19. [Query 19: Can you show me how customers are tracking against their spending limits?](#query-19)
    ...
    - *What it does:* Situation: The credit risk and customer service teams need real-time visibility into how cardholders are utilizing their available credit limits acros...
    - *Business Value:* [Expected output description]
...
20. [Query 20: Can you provide an overview of card management across multiple customer profiles?](#query-20)
    ...
    - *What it does:* Situation: Many customers maintain multiple card products (primary, authorized users, business cards, household members) that should be viewed holisti...
    - *Business Value:* [Expected output description]
...
21. [Query 21: Can you show me how our credit card offers are performing in terms of expiration tracking?](#query-21)
    ...
    - *What it does:* Situation: The credit card marketing team needs to monitor offer lifecycles to ensure timely renewals and prevent gaps in promotional campaigns. With...
    - *Business Value:* [Expected output description]
...
22. [Query 22: Can you provide an analysis of statement credits issued through our rewards program?](#query-22)
    ...
    - *What it does:* Situation: The rewards operations team is reviewing the financial impact of statement credit redemptions as customers increasingly prefer cash-back st...
    - *Business Value:* [Expected output description]
...
23. [Query 23: Can you help me optimize our card network distribution across different payment networks?](#query-23)
    ...
    - *What it does:* Situation: The product management team is evaluating the portfolio's card network distribution as processing fees and acceptance rates vary significan...
    - *Business Value:* [Expected output description]
...
24. [Query 24: Can you analyze the value proposition of our metal credit cards compared to standard plastic cards?](#query-24)
    ...
    - *What it does:* Situation: The product development team is debating whether to expand the metal card lineup given the significantly higher manufacturing costs ($15-20...
    - *Business Value:* [Expected output description]
...
25. [Query 25: Can you provide insights to optimize our business credit card offerings?](#query-25)
    ...
    - *What it does:* Situation: The business card division is underperforming compared to consumer cards, with lower activation rates and spend velocity. The team needs to...
    - *Business Value:* [Expected output description]
...
26. [Query 26: Show me the graduation rates and timelines for customers who started with secured credit cards.](#query-26)
    ...
    - *What it does:* Situation: The credit card operations team needs to evaluate the secured card program's effectiveness in helping customers build credit and transition...
    - *Business Value:* [Expected output description]
...
27. [Query 27: What's the impact of credit card usage on customer credit scores over time?](#query-27)
    ...
    - *What it does:* Situation: The customer success team wants to help cardholders understand how their credit card behavior affects their creditworthiness. Many customer...
    - *Business Value:* [Expected output description]
...
28. [Query 28: Which customers have rewards points expiring soon and how many points are at risk?](#query-28)
    ...
    - *What it does:* Situation: The rewards program manager has identified that millions of points expire unused each quarter, representing both lost customer value and mi...
    - *Business Value:* [Expected output description]
...
29. [Query 29: Compare the terms and benefits across our different credit card product agreements.](#query-29)
    ...
    - *What it does:* Situation: The product management team is reviewing the credit card portfolio to ensure competitive positioning and clear differentiation between card...
    - *Business Value:* [Expected output description]
...
30. [Query 30: Calculate an overall health score for our credit card portfolio across all key metrics.](#query-30)
    ...
    - *What it does:* Situation: Executive leadership needs a single, comprehensive metric to assess the overall health of the credit card business across multiple dimensio...
    - *Business Value:* [Expected output description]
...
**Description:** Situation: A credit card user is planning shopping trips or evaluating spending opportunities in their geographic area. They want to know which nearby merchants offer the best rewards potential when paired with their existing credit cards, and whether there are location-specific offers they should activate. The system contains merchant location data, card reward structures, offer details, and geospatial information needed for proximity-based optimization. Task: Generate a ranked list of nearby merchants with optimal card recommendations, calculate expected rewards for each merchant-card pairing, determine distance from the user's current location, and identify the activation status of applicable location-based offers. Action: The query performs geospatial joins to identify merchants within a specified radius, calculates distances using latitude/longitude coordinates, matches merchants with card reward rates and category bonuses, aggregates expected rewards based on historical or projec
...
**Description:** Situation: Credit card issuers frequently provide targeted offers such as cash back bonuses, statement credits, or elevated rewards at specific merchants. A user with multiple credit cards needs to track which offers are available across their portfolio, determine eligibility based on card type and account status, and prioritize which offers to activate to maximize savings given their typical spending patterns. The cards, offers, and transactions tables contain eligibility rules, offer terms, activation status, and spending history needed for this optimization. Task: Generate a comprehensive list of available offers with eligibility status across all cards, provide activation recommendations prioritized by potential savings, calculate estimated value based on spending patterns, track redemption progress toward offer thresholds, and create a prioritized activation checklist. Action: The query joins card accounts with available offers using eligibility criteria such as card product type
...
**Description:** Situation: The Federal Reserve publishes aggregate consumer credit data including revolving credit balances, delinquency rates, charge-off rates, and credit utilization across different consumer segments and time periods. Analyzing this macroeconomic data helps credit card users, financial advisors, and industry analysts understand broader market trends, identify how different demographic segments are managing credit, anticipate economic conditions that affect credit availability and pricing, and make informed decisions about credit card applications, balance management, and debt strategies in the current market environment. Task: Generate a comprehensive market trend analysis showing credit card debt levels over time, segment the market by consumer demographics and credit profiles, calculate delinquency and charge-off rates across segments, identify seasonal patterns in credit usage and payment behavior, and develop predictive indicators for future credit market conditions including e
...
**Description:** Situation: Credit card holders with multiple premium cards face significant annual fee burdens that may not be justified by the rewards and benefits earned. Evaluating whether each card's annual fee is offset by its value generation is essential for portfolio optimization and renewal decisions. Task: Calculate total annual fees paid per card, quantify value received from rewards earned and benefits utilized, compute net value (rewards minus fees) for each card, assess renewal worthiness based on cost-benefit ratios, and identify portfolio optimization opportunities to minimize fees while maximizing value. Action: The query aggregates annual fees by card and year, sums all rewards earned and redeemed per card using joins to rewards tables, calculates monetary value of benefits utilized (lounge access, credits, insurance), computes net value as total rewards plus benefits minus annual fees, uses window functions to compare year-over-year value trends, calculates break-even spending thres
...
**Description:** Situation: New credit card signups typically offer lucrative signup bonuses contingent on meeting minimum spend requirements within specified timeframes (usually 3-6 months). Cardholders managing multiple new cards need to track spend progress against these requirements to ensure bonus capture while optimizing spending allocation across cards. Task: Track all active signup bonus offers with their terms, monitor spending progress toward minimum spend thresholds on each card, calculate remaining spend needed and days remaining until deadlines, identify optimal spending allocation strategies across multiple cards with concurrent bonus periods, and provide timing recommendations for future applications to avoid overlapping spend requirements. Action: The query joins card opening dates with bonus terms to identify active bonus periods, aggregates transaction spending by card since opening date to calculate progress toward minimum spend, computes remaining spend needed and days until deadlin
...
**Description:** Situation: Credit card users want to maximize rewards by using the optimal card for each merchant based on category bonuses, promotional rates, and spending patterns. With multiple cards offering varying reward rates across merchant categories, selecting the best card for each transaction requires analyzing historical spending and current reward structures. Task: Analyze historical transaction data to identify top merchants by spending volume, match merchants to card reward categories, calculate effective reward rates for each card-merchant combination, generate predictive scores for reward optimization, and provide merchant-specific card recommendations ranked by expected reward value. Action: The query aggregates historical transactions by merchant to calculate total spending and frequency, joins merchant data with category mappings to identify applicable bonus categories, calculates effective reward rates per card-merchant pair accounting for base rates and category bonuses, uses wi
...
**Description:** Situation: The finance team is reviewing quarterly credit card expenses to identify cost reduction opportunities. With thousands of transactions across multiple categories, they need visibility into spending patterns to make informed budgeting decisions and identify categories where expenses are unusually high or could be consolidated. Task: Analyze transaction spending by category to identify optimization opportunities and spending anomalies. Action: The query joins the transactions table with cards and calculates aggregate metrics (total spend, transaction count, average transaction amount) grouped by spending category. It computes quartile distributions to identify outlier categories, applies window functions to calculate each category's percentage of total spend, and ranks categories by spend volume. Date range filters ensure only relevant periods are analyzed, and LEFT JOINs handle cases where category information may be missing. Result: The output returns each spending category w
...
**Description:** Situation: Credit utilization (the percentage of available credit being used) is a critical metric that affects credit scores and borrowing costs. The credit risk team needs to monitor utilization across the portfolio to identify customers using too much of their available credit (high utilization hurts credit scores and indicates potential default risk) or too little (underutilized credit represents opportunity costs). Optimal utilization is typically between 10-30% for score optimization. Task: Analyze credit limit utilization across the card portfolio and identify optimization opportunities through limit adjustments or balance management. Action: The query joins cards with current balance information, calculating utilization rate (current balance divided by credit limit) for each card. It groups accounts by utilization bands (0-10%, 10-30%, 30-50%, 50-100%, over 100%) and uses window functions to calculate rolling average utilization over 3, 6, and 12 months to identify trends. The
...
**Description:** Situation: The rewards program manager needs to understand how cardholders are utilizing their accumulated points across various redemption channels (cash back, travel, gift cards, statement credits) to optimize reward offerings and identify which redemption options drive the most engagement. Task: Analyze reward redemption patterns by computing total redemption values, average redemption amounts, conversion rates from points to currency, and identify trending redemption categories over time. Action: The query joins the rewards and transactions tables to link redemptions with earning patterns, groups by redemption type and time period (monthly or quarterly), calculates aggregate metrics including total points redeemed and dollar values, computes conversion rates and percentile distributions using window functions to identify high-value vs. low-value redemptions, and applies rolling averages to detect seasonal trends while handling NULL values for incomplete redemption records. Result:
...
**Description:** Situation: The partnerships team manages relationships with airline, hotel, and retail transfer partners where cardholders can convert reward points at various exchange rates, and needs visibility into which partnerships deliver the most value and customer engagement to inform contract renewals and identify underperforming partners. Task: Evaluate transfer partner utilization by measuring transfer volumes to each partner, calculating effective transfer rates, identifying the most and least popular partners, and analyzing customer transfer frequency and patterns. Action: The query joins rewards data with partner transfer tables and card profiles, groups by partner name and transfer destination category, computes total points transferred and number of unique customers using each partner, calculates average transfer amounts and effective exchange rates using aggregate functions, employs window functions to rank partners by popularity and compare quarter-over-quarter growth, and handles ca
...
**Description:** Situation: The customer retention and growth team wants to proactively identify cardholders whose spending behavior suggests they would derive more value from a different card tier—either upgrading high-spenders to premium cards with better rewards but higher fees, or downgrading low-utilization customers to no-fee cards to reduce churn—while maximizing customer lifetime value and satisfaction. Task: Identify upgrade and downgrade candidates by analyzing each cardholder's spending patterns, annual fee costs, rewards earned, and comparing their current card benefits against alternative card products to determine potential value gained or saved. Action: The query joins transactions, cards, and rewards tables to build comprehensive customer profiles, calculates total annual spending by category and rewards earned per customer, compares current annual fees against reward value generated, groups cardholders by current card tier, uses window functions to compute spending percentiles within e
...
**Description:** Situation: Many customers maintain multiple card products (primary, authorized users, business cards, household members) that should be viewed holistically for relationship management, and the account management team needs consolidated visibility across all linked profiles to understand total customer value, identify cross-sell opportunities, manage household credit exposure, and provide coordinated customer service. Task: Aggregate card activity across related profiles by identifying linked customer accounts, consolidating spending and rewards data across all associated cards, calculating total household relationship value, and presenting unified metrics. Action: The query joins multiple tables including customer profiles, card accounts, transactions, and rewards using hierarchical relationships (primary account holder, authorized users, household links), groups transactions and rewards by customer profile while maintaining card-level detail through CTEs or subqueries, calculates aggr
...
**Description:** Situation: The credit card marketing team needs to monitor offer lifecycles to ensure timely renewals and prevent gaps in promotional campaigns. With multiple offers across different card products, tracking expiration dates is critical to maintain customer acquisition momentum and avoid leaving cardholders without competitive offers. Task: Generate an offer expiration tracking report that identifies which offers are expiring soon and their current utilization status. Action: The query joins the offers table with cards and transactions data, filters for active offers, groups by offer ID and expiration date, calculates days until expiration using date functions, counts associated active cards per offer, and ranks offers by urgency using window functions ordered by expiration date. The query handles NULL expiration dates by treating them as indefinite offers. Result: The output returns a list of offers with their names, expiration dates, days remaining, number of active cards using each o
...
**Description:** Situation: The rewards operations team is reviewing the financial impact of statement credit redemptions as customers increasingly prefer cash-back style rewards over points or miles. Understanding redemption patterns helps forecast liabilities and optimize reward program profitability. Task: Create a statement credit analysis that quantifies redemption volumes, values, and patterns across different customer and card segments. Action: The query joins the rewards table with transactions and cards tables, filters for redemption type equal to statement credit, groups by card type, customer segment, and time period (monthly), computes aggregate metrics including total credits issued, average credit amount, count of redemptions, and calculates month-over-month growth rates using LAG window functions. The query also calculates redemption rate as a percentage of total rewards points earned. Result: The output delivers monthly statement credit metrics segmented by card type and customer tier,
...
**Description:** Situation: The product development team is debating whether to expand the metal card lineup given the significantly higher manufacturing costs ($15-20 per metal card vs $1-2 for plastic). Metal cards are perceived as premium products that attract high-value customers, but the financial justification needs quantification. Task: Create a metal card value analysis that compares key performance metrics between metal and plastic card segments to assess ROI. Action: The query segments cards by material type (metal vs plastic) using a CASE statement, joins with transactions and customer tables, groups by card material and card product, calculates average monthly spend per card, annual fee revenue, retention rate after year one, and customer lifetime value using a CLV formula with discount rate. The query uses window functions to compute percentile rankings of metal cardholders within the overall customer base and calculates the incremental value (difference) between metal and plastic segments
...
**Description:** Situation: The customer success team wants to help cardholders understand how their credit card behavior affects their creditworthiness. Many customers don't realize that high utilization rates or missed payments significantly impact credit scores, while responsible usage can improve scores over time. The bank needs data-driven insights to provide personalized guidance and identify at-risk customers before score deterioration. Task: Measure the relationship between credit card activity (utilization rates, payment timeliness, account age) and credit score changes over rolling time periods for active cardholders. Action: The query joins credit score snapshots with transaction and payment history tables, calculates utilization rates as balance divided by credit limit by month, segments customers into utilization bands (low, medium, high), uses window functions to compute score changes over 3, 6, and 12-month periods, groups by utilization band and payment status, and calculates average sc
...
**Description:** Situation: The rewards program manager has identified that millions of points expire unused each quarter, representing both lost customer value and missed engagement opportunities. Customers often don't realize their points have expiration dates, leading to dissatisfaction when they discover expired balances. The program needs a proactive alert system to notify customers before expiration and encourage redemption. Task: Identify all rewards points approaching expiration within the next 30, 60, and 90 days, quantify the value at risk by customer and program tier, and prioritize customers for outreach based on points balance and expiration urgency. Action: The query filters the rewards table for points with expiration dates in future windows, groups by customer ID and expiration date, sums points by expiration bucket (30/60/90 days), joins with customer profile to get contact information and tier status, calculates total point value at risk using redemption rates, and uses window functio
...
**Description:** Situation: Executive leadership needs a single, comprehensive metric to assess the overall health of the credit card business across multiple dimensions including credit risk, customer engagement, profitability, and growth. Currently, teams report isolated metrics (delinquency rates, transaction volume, NPS scores) without integration, making it difficult to understand holistic portfolio performance or compare performance across regions and product lines. A unified health score would enable faster decision-making and consistent performance tracking. Task: Develop a weighted composite health score (0-100 scale) that combines credit quality metrics, utilization and engagement indicators, profitability measures, customer satisfaction scores, and growth rates, with the ability to drill down by product type, customer segment, and geography. Action: The query constructs a multi-layered calculation using CTEs: first CTE computes credit metrics (delinquency rate, charge-off rate, utilization)
