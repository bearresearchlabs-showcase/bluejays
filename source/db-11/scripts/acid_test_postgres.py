#!/usr/bin/env python3
"""
db-11 ACID Compliance Test and Query Execution
...
def get_pg_schema_for_postgres(content: str) -> str:
    """Convert schema for PostgreSQL: TIMESTAMP_NTZ -> TIMESTAMP, ensure PostGIS"""
...
def setup_docker_postgres() -> bool:
    ...
    try:
        ...
        if 'db11_postgres' in result.stdout:
            return True
        # Start container
        subprocess.run([
            'docker', 'run', '-d', '--name', 'db11_postgres',
            '-e', 'POSTGRES_PASSWORD=postgres',
            '-e', 'POSTGRES_DB=db11_acid_test',
            '-p', '5433:5432',
...
def get_connection(use_docker: bool = False, database: str = 'db11_acid_test') -> Optional[Any]:
    """Get PostgreSQL connection"""
    if not POSTGRES_AVAILABLE:
        return None
    port = int(os.getenv('PG_PORT', '5433' if use_docker else '5432'))
    try:
...
def run_acid_tests(conn) -> Dict[str, Any]:
    ...
    try:
        ...
        cursor.execute("INSERT INTO data_source_metadata (source_id, source_name, source_type) VALUES ('acid_test_1', 'ACID Test', 'Test')")
        cursor.execute("ROLLBACK")
        cursor.execute("SELECT COUNT(*) AS cnt FROM data_source_metadata WHERE source_id = 'acid_test_1'")
    ...
    try:
        cursor.execute("BEGIN")
        try:
            cursor.execute("INSERT INTO cities (city_id, city_name, state_code, msa_id) VALUES ('acid_test', 'Test', 'XX', 'nonexistent_msa')")
            results['tests']['consistency'] = {'Pass': 0, 'note': 'FK violation should have been rejected'}
        except psycopg2.IntegrityError:
            conn.rollback()
            results['tests']['consistency'] = {'Pass': 1, 'note': 'FK constraint enforced'}
    except Exception as e:
    ...
    try:
        cursor.execute("BEGIN")
        cursor.execute("INSERT INTO data_source_metadata (source_id, source_name, source_type) VALUES ('acid_durability', 'Durability Test', 'Test')")
        conn.commit()
        cursor.execute("SELECT COUNT(*) AS cnt FROM data_source_metadata WHERE source_id = 'acid_durability'")
        cnt = cursor.fetchone()['cnt']
        cursor.execute("DELETE FROM data_source_metadata WHERE source_id = 'acid_durability'")
        conn.commit()
...
def run_queries(conn, queries: List[Dict], limit: int = 10) -> List[Dict]:
...
def main():
    print("=" * 70)
    print("db-11 ACID Compliance Test and Query Execution")
    print("=" * 70)

    use_docker = os.getenv('DB11_USE_DOCKER', '') == '1'
    if use_docker:
        print("\nUsing Docker PostgreSQL...")
        setup_docker_postgres()

    # Try postgres first for db creation, then db11_acid_test
    conn = get_connection(use_docker, database='postgres')
    if not conn:
        conn = get_connection(use_docker, database='db11_acid_test')
    if not conn:
        print("\nCould not connect to PostgreSQL. Ensure PostgreSQL is running.")
        print("Options:")
        print("  1. Set PG_HOST, PG_PORT, PG_USER, PG_PASSWORD")
        print("  2. Or use: DB11_USE_DOCKER=1 to start via Docker (postgis/postgis:16-3.4)")
        return 1
    ...
    try:
        with conn.cursor() as cur:
            cur.execute("SELECT 1 FROM pg_database WHERE datname = 'db11_acid_test'")
            if not cur.fetchone():
                cur.execute("CREATE DATABASE db11_acid_test")
                print("Created database db11_acid_test")
    except Exception as e:
