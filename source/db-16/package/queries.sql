-- ============================================================================
-- db-16 Flood Risk Assessment Database - SQL Queries
-- M&A Due Diligence Workflows for Large Real Estate Firms
-- ============================================================================
--
-- Database: db-16
-- Purpose: Flood Risk Assessment for M&A Acquisition Target Evaluation
-- Total Queries: 30
-- Compatibility: PostgreSQL
--
-- All queries are designed for M&A due diligence workflows:
--   - Pre-acquisition property risk assessment
--   - Portfolio-level risk analysis
--   - Financial impact modeling for deal pricing
--   - Historical risk pattern analysis
--   - Multi-horizon risk projections
--
-- ============================================================================
...
-- Business Value: Comprehensive pre-acquisition flood risk assessment report for individual properties showing current risk, short-term (5-10 years), intermediate-term (20-30 years), and long-term (50-100 years) projections. Enables M&A teams at large real estate firms to make informed go/no-go acquisition decisions based on location-specific flood vulnerability analysis. Provides critical due diligence data for acquisition pricing negotiations and deal structuring.
-- Purpose: Quantifies flood risk across multiple time horizons and data sources for properties under consideration for acquisition, enabling data-driven M&A decision-making and acquisition target risk evaluation.
...
fema_flood_zone_analysis AS (
    ...
    SELECT
        ...
        CASE
            WHEN ffz.zone_geom IS NOT NULL AND pla.property_geom IS NOT NULL THEN
                CASE
                    WHEN ST_WITHIN(pla.property_geom, ffz.zone_geom) THEN TRUE
                    ELSE FALSE
                END
            ELSE NULL
...
usgs_streamflow_analysis AS (
    ...
    SELECT
        ...
        -- Get historical flood frequency
        (
            SELECT COUNT(*)
            ...
            WHERE usg.gauge_geom IS NOT NULL
                AND usg.active_status = TRUE
                AND slrs.property_geom IS NOT NULL
                AND ST_DISTANCE(usg.gauge_geom, slrs.property_geom) < 10000
    ...
    SELECT
        ...
        CASE
            WHEN hfe.total_damage_dollars IS NOT NULL AND hfe.properties_affected IS NOT NULL THEN
                LN(COALESCE(hfe.total_damage_dollars, 1) + 1) * 
                LN(COALESCE(hfe.properties_affected, 1) + 1) * 
                COALESCE(hfe.fatalities, 0) + 1
            WHEN hfe.total_damage_dollars IS NOT NULL THEN
                LN(COALESCE(hfe.total_damage_dollars, 1) + 1) * 10
            WHEN hfe.properties_affected IS NOT NULL THEN
                LN(COALESCE(hfe.properties_affected, 1) + 1) * 10
            ELSE 1
...
recurrence_interval_analysis AS (
    SELECT
        ...
        PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY ffc.peak_discharge_cfs) OVER (PARTITION BY ffc.gauge_id) AS median_peak_discharge
...
gauge_network_coverage AS (
    SELECT
        ...
        ROUND(CAST(ria.median_peak_discharge AS NUMERIC), 2) AS median_peak_discharge_cfs,
        (
            SELECT COUNT(*)
...
WITH property_spatial_network AS (
    ...
    INNER JOIN fema_flood_zones ffz1 ON (
        ...
        AND ST_WITHIN(rep1.property_geom, ffz1.zone_geom)
    )
        ...
        CASE
            WHEN EXISTS (
                ...
                WHERE ffz.zone_geom IS NOT NULL
                    ...
                    AND ST_WITHIN(rep.property_geom, ffz.zone_geom)
            ) THEN TRUE
            ELSE FALSE
...
recursive_risk_propagation AS (
    ...
    SELECT
        ...
        pbr.state_code,
        pbr.county_fips,
        pbr.base_risk_score AS propagated_risk_score,
        pbr.base_risk_category AS propagated_risk_category,
        pbr.base_risk_score AS cumulative_risk_score,
        0 AS propagation_depth,
        ARRAY[pbr.property_id] AS propagation_path,
    ...
    SELECT
        ...
        rrp.propagation_path || pbr.property_id AS propagation_path,
        pbr.is_in_flood_zone
