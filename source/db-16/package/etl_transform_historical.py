...
def transform_historical_record(record: Dict[str, Any]) -> Dict[str, Any]:
    ...
    transformed = {
        'event_id': event_id,
        'event_name': record.get('place', 'Flood Event')[:255],
        'event_type': record.get('type', 'Flood')[:50],
        'start_date': dt.isoformat() if dt else None,
        'end_date': None,
        'affected_area_geom': event_geom,
        'peak_discharge_cfs': None,
        'peak_stage_feet': parse_float(record.get('depth')),
        'total_damage_dollars': float(damage_estimate) if damage_estimate else None,
        'fatalities': None,
        'properties_affected': None,
        'state_code': None,
        'county_fips': None,
