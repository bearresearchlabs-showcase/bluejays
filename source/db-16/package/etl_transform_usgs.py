...
def transform_observation_record(record: Dict[str, Any], gauge_id: str, 
    ...
    stage_feet = parse_float(record.get('gage_height_va'))
    percentile_rank = None
    if gage_height and stages.get('flood_stage'):
        if gage_height >= stages.get('flood_stage'):
            percentile_rank = 90.0
        elif gage_height >= stages.get('flood_stage', 0) * 0.8:
    ...
    transformed = {
        ...
        'gage_height_feet': gage_height,
        'discharge_cfs': discharge,
        'stage_feet': stage_feet,
        'flood_category': flood_category,
        'percentile_rank': percentile_rank,
        'data_quality_code': record.get('gage_height_cd', '')[:10],
        'load_timestamp': datetime.now().isoformat()
    }
