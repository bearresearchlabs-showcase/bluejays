...
```bash
psql -U postgres -d db_14 -f DATABASE/schema.sql
```
...
```bash
psql -U postgres -d db_14 -f DATABASE/data.sql
```
...
See `QUERIES/queries.md` for 30 production SQL queries with full business context, evidence, and expected output. Queries cover cost comparison, rightsizing, optimization recommendations, and FinOps analytics.
