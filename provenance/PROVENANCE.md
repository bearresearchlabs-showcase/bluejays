...
### The conditions, stated up front

Everything below should be read against four facts, confirmed with the builder and
corroborated by the file timestamps:

| | |
| --- | --- |
| **No reference benchmark** | Nothing to copy the conventions from — no exemplar field schema, no difficulty scale, no ground-truth format. The BIRD/LiveSQLBench alignment is a reconstruction, not a spec that was followed. |
| **No acceptance rubric** | None existed at any point. Acceptance was one sentence of client impression, after the fact. |
| **Requirements emerged mid-build** | The SOW is dated **four days before** the last instance file was written. |
| **A 17-day clock** | 2026-02-01 (first bundle in the delivery format) → 2026-02-18 (last instance written). Fourteen days from the first pathfinder. Packaging followed 19 days later. |

**75 of the 97 shipped files were written on the final day.**

Thirteen environments, 176 tables, 390 annotated query pairs and 19.4 GB of data, in 17 days,
with no reference artifact and no acceptance criterion. That is the achievement. The defects
in §6 are the predictable cost of the method, not evidence about care — and §18 argues they
are close to exactly the set of conventions a reference benchmark would have supplied free.
...
at the end of the current statement. The two outliers are the two pathfinders, generated
before the cap was applied.

> **Correction, confirmed 2026-08-06.** An earlier version of this document read the 1 GiB
> floor as a deliberate design choice. It was not. It was a **size cap imposed by the
> delivery channel**, applied because this package is a **sample** rather than a complete
> corpus. The evidence is consistent with that reading: a design parameter would have been
> applied retroactively to the two pathfinders; a delivery cap was not.
>
> This matters beyond the correction itself. **The shipped package is a sample**, cut to fit
> how it had to be sent. Absolute scale claims describe this delivery, not the method — the
> generators produce arbitrarily more rows. And the audit findings in §6 are properties of
> the sample, which is the only thing that shipped and therefore the only thing that can be
> checked.

The uniformity still buys two things regardless of why it exists: per-database execution cost
is roughly constant, so no environment silently dominates a training run on wall-clock; and
no table is small enough to be memorized whole. Properties of the artifact, not achievements
of its design.
...
**Every one of these questions is unanswered in the artifacts — and one is now answered
definitively: there was never an acceptance rubric.** Not a lost one, not an informal one.
None. Combined with the absence of a reference benchmark, that means the entire quality
standard for this engagement was a single sentence of reviewer impression delivered after
delivery. Until a rubric exists, a rewrite is a guess with a cost attached. §14.5 is the concrete illustration: the client's
...
There is a sharper version of this, and it is the one worth taking into the next
conversation. **The six failing invariants in §6 are not a random sample of possible
defects.** Sort them by what a reference benchmark would have supplied for free:

| | Failure | What Spider/BIRD supply without anyone deciding to |
| --- | --- | --- |
| 6.4 / I6 | `expected_output` is prose | Executed result sets — the format *is* the ground truth |
| 6.3 / I5 | `complexity` constant | A difficulty scale ships with the corpus |
| 6.2 / I4 | 1,053 near-duplicate pairs | No corpus states a distinctness rule; you only need one if you generate 30 queries per schema on a clock |
| — / I3 | db-3's undeclared table | The first execution-harness run catches it |

Every one of those exists in Spider and BIRD as a **consequence of their evaluation
harnesses**, not as a stated design rule. Build against those corpora and you absorb them
without noticing. Build with no reference, on a 17-day clock, while requirements are still
arriving — and you have to derive each one from first principles. The ones that get dropped
are exactly the ones that stay invisible until something tries to *score* the artifact.

That is a genuine finding, it generalizes past this engagement, and it is the honest frame
for what happened here: not carelessness, but the predictable signature of improvising a
benchmark without a harness. The practical corollary — build the harness first, even a
trivial one — is also the thing that would most improve the next batch.
