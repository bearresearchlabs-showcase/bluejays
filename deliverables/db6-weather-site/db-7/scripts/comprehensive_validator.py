#!/usr/bin/env python3
"""
Comprehensive validation script for db-7 queries.md
Performs syntax validation, execution testing, and comprehensive evaluation
Phase 2 & 4: Syntax Validation and Comprehensive Evaluation for db-7 Maritime Shipping Intelligence Database
"""
...
class QueryEvaluator:
    ...
    def evaluate_recursive_cte_usage(queries: List[Dict]) -> Dict:
        ...
        results = []
        for query in queries:
            # Check for WITH RECURSIVE anywhere in the SQL
        ...
        return {
            ...
            'Pass': 1 if len(mismatched) == 0 else 0,
            'details': results,
    ...
    def evaluate_cte_usage(queries: List[Dict]) -> Dict:
        ...
        return {
            ...
            'Pass': 1 if len(queries_without_cte) == 0 else 0,
    ...
    def evaluate_complexity(queries: List[Dict]) -> Dict:
        ...
        return {
            ...
            'Pass': 1 if avg_cte >= 1.0 and avg_join >= 1.0 else 0  # Minimum complexity requirement
        }
...
def main():
    ...
    print("="*70)
    print("Comprehensive Validation for db-7 Maritime Shipping Intelligence queries.md")
    print("="*70)
    ...
    pg_config = {
        ...
        'database': os.environ.get('PG_DATABASE', 'db_7_validation')
    }
    ...
    if not all([db_config.get('server_hostname'), db_config.get('http_path'), db_config.get('token')]):
    ...
    # Check if any databases were available (for informational purposes only)
    # Note: Evaluation phase will still run even without database connections
    pg_available = results['syntax_validation'].get('postgresql', {}).get('available', False)
    db_available = results['syntax_validation'].get('databricks', {}).get('available', False)
    if not pg_available and not db_available:
        print("\n⚠️  No database connections available. Continuing with evaluation phase only.")
    ...
    status_text = "PASS" if count_result['Pass'] == 1 else "FAIL"
    print(f"\nQuery Count: {count_result['found']}/30 - {status_text}")
    ...
    status_text = "PASS" if cte_result['Pass'] == 1 else "FAIL"
    print(f"CTE Usage: {cte_result['queries_with_cte']}/{cte_result['total_queries']} queries use CTEs - {status_text}")
    ...
    status_text = "PASS" if complexity_result['Pass'] == 1 else "FAIL"
    print(f"Complexity: Avg {complexity_result['average_cte_count']:.1f} CTEs, "
          f"{complexity_result['average_join_count']:.1f} joins, "
          f"{complexity_result['average_window_function_count']:.1f} window functions per query - {status_text}")
    results['evaluation']['complexity'] = complexity_result

    # Overall Pass status
    evaluation_pass = all(
        result.get('Pass', 0) == 1
        for result in results['evaluation'].values()
        if isinstance(result, dict) and 'Pass' in result
    )
    results['Pass'] = 1 if evaluation_pass else 0
    ...
    overall_status = "PASS" if results['Pass'] == 1 else "FAIL"
