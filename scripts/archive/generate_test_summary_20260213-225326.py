...
def analyze_results(results):
    ...
    for db_name, db_results in results.items():
        ...
        if sf_stats.get('available', False):
            ...
            summary['overall']['databricks']['total_tested'] += db_summary['databricks']['total_queries']
            summary['overall']['databricks']['successful'] += db_summary['databricks']['successful']
            summary['overall']['databricks']['failed'] += db_summary['databricks']['failed']
        else:
            db_summary['databricks'] = {
                'available': False,
                'reason': sf_stats.get('reason', 'Databricks connector not available or connection failed')
            }
    ...
    if summary['overall']['databricks']['total_tested'] > 0:
        summary['overall']['databricks']['success_rate'] = (
            summary['overall']['databricks']['successful'] /
            summary['overall']['databricks']['total_tested'] * 100
        )
...
def print_summary(summary):
    ...
    for db_name, db_data in summary['databases'].items():
        ...
        sf_data = db_data.get('databricks', {})
        if sf_data.get('available'):
            ...
            print(f"  Databricks: ❌ {sf_data.get('reason', 'Not available')}")
